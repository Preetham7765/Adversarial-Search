...
def convert_to_str(board):
    ...
    for i in range(0,8):
        for j in range(0,8):
            next_move_str += board[i][j]
    return next_move_str

def isValidForParakeeth(board, row, col, i,p):
...
def move_parakeeth(board,p,row,col):
    ...
    for i in range(0, len(Parakeeth_Row)):
        ...
        if (isValidForParakeeth(board, row1, col1, i,p)):
...
def max_value(state,depth,alpha,beta,player):
    ...
    if(depth == maxdepth ):
        ...
        for successor in generatesuccessor(state, player):
            successor_str = convert_to_str(successor)
            if successor_str in visited:
                alpha = visited[successor_str]
            else:
                alpha = max(alpha,mini_value(successor, depth+1, alpha, beta, not player))
                visited[successor_str] = alpha
            if(alpha >= beta):
                return alpha
    return alpha
...
def mini_value(state,depth,alpha,beta,player):
    ...
    if(depth == maxdepth):
        ...
        if(player):
            ...
            return -1 * (piece_square_evaluation(state)+material_evaluation(state))
    else:
        for successor in generatesuccessor(state, player):
            successor_str = convert_to_str(successor)
            if successor_str in visited:
                beta = visited[successor_str]
            else:
                beta = min(beta,max_value(successor, depth+1, alpha, beta, not player))
                visited[successor_str] = beta
            if(alpha >= beta):
                return beta
    return beta
...
def main():
    ...
    print("**************************************************************************************\n")
    print(convert_to_str(next_move))
